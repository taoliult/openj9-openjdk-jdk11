...
public class SecureRandom extends java.util.Random {
    ...
    private void getDefaultPRNG(boolean setSeed, byte[] seed) {
        ...
        for (Provider p : Providers.getProviderList().providers()) {
            // In security restrict mode, use the SecureRandom from security restrict provider
            if (SecurityRestrictConfigurator.enableSecurityRestrict()) {
                String srProvider = SecurityRestrictProperties.getInstance().getJdkSecureRandomProvider();
                if (p.getName().equals(srProvider)) {
                    prngAlgorithm = SecurityRestrictProperties.getInstance().getJdkSecureRandomAlgorithm();
